...
class SellForm extends Component {
	...
	render() {
		return (
			<form className="mb-3" onSubmit={(event) => {
				...
				if(this.state.tokenRate === '1000') {
					this.props.sellDogTokens(etherAmount);
				}
				
				if(this.state.tokenRate === '100') {
					this.props.sellSheepTokens(etherAmount);
				}				

				if(this.state.tokenRate === '10') {
					this.props.sellPigTokens(etherAmount);
				}
			}}>
			<div>
			...
			<div className="input-group mb-4">
				...
				<input
					...
					onChange={(event) => {
						...
						this.setState({
							output: tokenAmount / this.state.tokenRate
						})
					}}
        ...
        <div className="mb-5">
          ...
          <span className="float-right text-muted">10 Pig Tokens = 100 Sheep Tokens = 1000 Dog Tokens = 1 ETH</span>
        </div>
